...
- 当前状态：Phase 1.11 用户侧数据保全与发布观测主线已完成；Phase 1.11.0 数据保全基线与文档分类、Phase 1.11.1 本地历史版本 v1、Phase 1.11.2 数据恢复中心 v1、Phase 1.11.3 危险写入保护、Phase 1.11.4 同步误覆盖防护、Phase 1.11.5 云端历史版本 v1、Phase 1.11.6 账号托管可恢复模型收口、Phase 1.11.7 P0 回归测试与事故演练、Phase 1.11.8 基础埋点、Phase 1.11.9 错误监控已完成。
- 优先级：P0，高于隐私、防泄露、组件优化和 UI 打磨。
- 需求背景：经历首页数据被清空/误覆盖事故后，产品规划必须先保证用户数据不会丢、可追踪、可预览、可恢复。
- 阶段定位：Phase 1.11 建立本地历史版本、云端历史版本、数据恢复中心、危险写入保护和同步误覆盖防护；后台管理 dashboard 已延期到 Phase 1.18，在商业化正式域名上线后再做。
...
- 后续阶段继续覆盖：恢复默认、应用空白页、应用模板、数据包恢复、收藏/URL 导入确认写入、空间切换、云端拉取覆盖本地；管理员查看、预览和恢复辅助延期到 Phase 1.18 后台 dashboard。

## 后台管理 Dashboard

状态：延期到 Phase 1.18。该能力需要受控服务端入口、管理员身份、管理员审计和 service role 隔离，复杂度明显高于 Phase 1.11 用户侧数据保全功能。完整方案已单独保存到 `docs/backlog/AdminDashboardBacklog.md`。
...
| Phase 1.11.8 | 基础埋点 | 已实现 `product_analytics_events`、受控上报 RPC、前端埋点 facade、设置页关闭入口和使用指南；只采集白名单产品事件和脱敏属性 | M | Medium-High |
| Phase 1.11.9 | 错误监控 | 已实现 `client_error_events`、受控上报 RPC、前端错误监控 facade、全局异常捕获、错误兜底 UI、关键异步失败上报、设置页关闭入口和使用指南 | M | Medium |
| Phase 1.18 | Supabase 后台管理 dashboard | 商业化正式域名上线后再做；`admin_users`、`admin_audit_events`、受控后台入口、非离线加密数据审计、管理员访问留痕 | XL | High |

推荐实施顺序：Phase 1.11.0-1.11.9 已完成，已先阻断用户端误覆盖、补齐账号托管云端历史、收口账号托管恢复模型，固化 P0 回归测试与事故演练，并补齐基础产品埋点和错误监控。后台管理 dashboard 延期到 Phase 1.18。
...
- 管理员 dashboard 如果实现不当，风险高于普通产品功能，必须严格隔离 service role；该能力已延期到 Phase 1.18。
...
- Phase 1.18 后台 dashboard 上线后，管理员查看非离线加密数据时必须写入管理员审计日志。
- P0 回归用例已覆盖首页清空、恢复默认、模板应用、批量导入、数据包恢复、多设备同步覆盖和冲突解决，并沉淀到 `docs/guides/DataPreservationP0RegressionDrill.md`。
