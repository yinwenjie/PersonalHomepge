...
| Phase 1.11 | P0 | 进行中 | 数据保全与恢复体系 | 已建立本地/云端历史版本、数据恢复中心、危险写入保护和同步误覆盖防护；后台管理 dashboard 延期到 Phase 1.14 | L-XL | High |
| Phase 1.12 | P1 | 延后 | 组件设计和优化 | 复盘 Todo、月历、组件管理模式和模板默认组件，优化配置入口、视觉密度、移动端布局和后续组件候选 | M-L | Medium |
