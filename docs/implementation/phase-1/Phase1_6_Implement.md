...
原 Phase 1.9 正式推出前收口整体后移到 Phase 1.10；后续计划调整后，Phase 1.10 已完成数据导入/恢复、操作审计、设备状态和同步请求多标签协调 MVP；Phase 1.11 调整为数据保全与恢复体系，组件设计和优化后移到 Phase 1.12，主域名准备进一步延期到 Phase 1.13。
...
| Phase 1.11 | P0-P1 | 进行中 | 数据保全与发布观测体系 | 已建立本地/云端历史版本、数据恢复中心、危险写入保护、同步误覆盖防护、账号托管恢复边界、P0 回归演练指南和基础埋点；错误监控提前为 Phase 1.11.9；后台管理 dashboard 延期到 Phase 1.14 | L-XL | High |
| Phase 1.12 | P1 | 后移 | 组件设计和优化 | 复盘 Todo、月历、组件管理模式和模板默认组件，优化配置入口、视觉密度、移动端布局和后续组件候选 | M-L | Medium |
| Phase 1.13 | P2 | 延后 | 主域名准备 | 域名购买决策完成后，在正式推出前处理自购主域名、Auth redirect、`basePath` 和缓存隔离回归 | M | High |
