...
- 设置页“首页空间”列表支持 `设默认`、`重命名` 和 `移除`。
- `设默认` 只更新账号默认空间，不拉取云端首页，也不覆盖当前浏览器本地首页。
- `重命名` 只更新账号空间名称，不影响同步码、账号托管凭证或首页内容。
- `移除` 表示“从账号移除空间索引”：
  - 普通 `sync-code` 空间移除后，同步码本身和云端密文内容不删除。
  - 账号托管空间移除后，对应 `home_space_credentials` 会随账号空间索引级联删除，空白设备不能再通过账号恢复该空间。
  - 移除不废弃底层 `sync_spaces`，不做密钥轮换。
...
- `useAccountData` 新增重命名、设默认、移除状态和错误消息。
...
- 登录用户可从账号移除非当前账号托管空间。
- 移除普通同步码空间后，旧同步码仍可绑定、拉取、上传。
- 移除账号托管空间后，账号空间列表不再显示该空间，空白设备不能再通过账号恢复它。
- 当前本机账号托管空间的 `移除` 按钮禁用，并提示先解除本机或切换空间。
- 移除默认空间后，默认空间设置被清空，不产生悬空默认标记。
