...
截至 Phase 1.6.7b，账号托管同步主链路已经形成，并已补齐账号级通用偏好编辑、Beta 状态统一和安全数据导出：
...
Phase 1.6.6 新增了账号偏好字段迁移；Phase 1.6.7 和 1.6.7b 仅做前端状态收口和安全导出。它们都没有改变同步码协议或首页文档 schema。由于主域名尚未决定，原主域名准备后移到 Phase 1.9 正式推出前收口；Phase 1.6 的剩余重点调整为模板库 v1。

Phase 1.6.8 模板库 v1 的入口范围：

- 空白浏览器首次启动时，以模板库作为首页初始化入口。
- 空白用户首次登录时，如果没有可直接恢复的当前首页，引导从模板创建新首页。
- 用户新创建首页空间时，可以从模板库选择起点，而不是只创建空白或默认首页。
- 模板先作为前端静态配置，应用后生成普通 `HomeDocumentV2`；不新增 Supabase 表，不引入模板收藏。
- 应用模板必须有覆盖确认，避免误覆盖已有首页。

Phase 1.6.9 浏览器收藏/标签导入 v1 紧随模板库之后实现。该能力属于 Phase 1 核心导入路径，但需要遵守浏览器权限边界：

- 普通网页不能直接读取浏览器收藏夹或当前打开标签页。
- Phase 1 支持导入浏览器导出的书签 HTML 文件，解析为首页分组和网站。
- Phase 1 支持粘贴标签页 URL 列表，解析为候选网站。
- 真正一键读取收藏夹或标签页需要浏览器扩展权限，后续进入 Phase 2 或独立扩展路线评估。
- 导入结果先本地预览，展示数量、重复项、非法 URL 和目标分组；用户确认后才写入 `HomeDocumentV2.groups`。
- 默认使用追加或新建分组，不静默覆盖当前首页内容。
...
| Phase 1.6.8 | P1 | 待实现 | 模板库 v1 | 空白浏览器首次启动、空白用户首次登录和新建首页空间时，以模板库作为创建入口 | M | Medium |
| Phase 1.6.9 | P1 | 待实现 | 浏览器收藏/标签导入 v1 | 导入浏览器导出的书签 HTML，或粘贴标签页 URL 列表，预览确认后生成首页分组和网站 | M | Medium |
| Phase 1.9 | P1 | 延后 | 主域名准备 | 域名购买决策完成后，在正式推出前处理自购主域名、Auth redirect、`basePath` 和缓存隔离回归 | M | High |
