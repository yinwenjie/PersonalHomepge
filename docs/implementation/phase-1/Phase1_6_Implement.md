...
- 已登录用户的同步码面板改为 `高级同步码与恢复`，默认折叠同步码创建、绑定和旧空间维护操作。
- 当前浏览器绑定账号托管空间时，高级同步码面板不显示完整同步码，也不提供废弃入口；只保留同步状态、手动拉取/上传、解除本机和输入同步码恢复其他空间等必要能力。
...
- 账号托管空间在高级同步码面板中隐藏完整凭证和废弃操作，避免把账号托管空间误理解为普通同步码空间。
...
Phase 1.6.5a 不改变同步码协议和数据库结构。本阶段补强“高级同步码与恢复”和“首页空间”的管理边界，避免用户把解除本机、废弃同步码、从账号移除和账号托管恢复凭证混为同一类操作。

用户侧变化：

- `高级同步码与恢复` 能识别当前本机绑定是否已经属于账号首页空间。
...
- 支持编辑：
  ...
  - 字体：系统默认、衬线、等宽。
  - 界面密度：舒适、紧凑。
  - 默认搜索引擎：DuckDuckGo、Google、Bing、Baidu。
...
- 新增枚举型 check 约束，限制：
  ...
  - `default_search_engine`: `duckduckgo | google | bing | baidu`
...
截至 Phase 1.6.6，账号托管同步主链路已经形成，并已补齐账号级通用偏好编辑：

- 登录用户可以创建账号托管空间，并在空白设备登录后恢复。
- 已认领普通同步码空间可以迁移为账号托管，旧同步码仍保留有效。
- 首页空间支持重命名、设默认和从账号移除。
- 删除策略已固定为账号侧移除，不删除或废弃底层 `sync_spaces`。
- 登录用户默认通过 `首页空间` 管理同步，普通同步码能力收起到 `高级同步码与恢复`。
- 同步码管理边界已补强，`解除本机`、`废弃同步码`、`从账号移除` 和账号托管凭证在 UI 文案和确认弹窗中明确区分。
- 全局偏好支持账号保存和未登录本地兜底，低风险展示偏好已在首页生效。

Phase 1.6.6 新增了账号偏好字段迁移，但仍没有改变同步码协议或首页文档 schema。后续 Phase 1.6 的剩余重点转向空间新同步码、导出和部署域名回归。

## Phase 1.6 后续拆分

| 子阶段 | 状态 | 目标 | 主要交付 |
|---|---|---|---|
| Phase 1.6.1 | 已完成 | 账号托管空间创建 | 已接入 `create_account_managed_home_space`，登录用户可创建账号托管空间 |
| Phase 1.6.1a | 已完成 | 恢复默认同步保护 | 已新增 `paused` 同步状态，恢复默认后暂停自动同步并提供上传默认、拉取云端、解除本机、恢复备份等选择 |
| Phase 1.6.2 | 已完成 | 空白设备账号恢复 | 已支持登录后读取 `home_space_credentials` 并恢复账号托管空间 |
| Phase 1.6.3 | 已完成 | 同步码迁移为账号托管 | 已支持当前已认领同步码空间原地迁移为账号托管；旧同步码保留有效 |
| Phase 1.6.4 | 已完成 | 首页空间 CRUD | 已支持重命名、设默认和从账号移除；不删除底层 `sync_spaces` |
| Phase 1.6.4a | 已完成 | 删除策略收口 | 已统一“从账号移除”语义、禁用当前本机账号托管空间移除，并补充删除策略验证脚本 |
| Phase 1.6.5 | 已完成 | 同步码入口降级 | 已将登录用户的同步码操作折叠到高级/恢复区域，首页空间成为账号用户主入口 |
| Phase 1.6.5a | 已完成 | 同步码管理边界补强 | 已补强解除本机、废弃同步码、输入同步码恢复与账号首页空间之间的边界提示 |
| Phase 1.6.6 | 已完成 | 全局偏好编辑 | 已支持账号偏好编辑、未登录本地偏好兜底、主题/字体/密度/日期/默认搜索引擎生效 |
| Phase 1.6.6a | 待实现 | 为空间生成新同步码 | 已有首页空间可以生成新的普通同步码，用于恢复、迁移或旧码废弃后的补救 |
| Phase 1.6.7 | 待实现 | Beta 质量补齐 | 空状态、错误状态、保存/同步状态和数据导出 |
| Phase 1.6.8 | 待实现 | 主域名准备 | 自购主域名、Auth redirect、`basePath` 和缓存隔离回归 |
