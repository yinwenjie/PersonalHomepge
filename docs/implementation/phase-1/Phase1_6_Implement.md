...
原 Phase 1.9 正式推出前收口整体后移到 Phase 1.10；后续计划调整后，Phase 1.10 已完成数据导入/恢复、操作审计、设备状态和同步请求多标签协调 MVP；Phase 1.11 调整为数据保全与恢复体系，组件设计和优化后移到 Phase 1.12，产品化体验收口进入 Phase 1.13，主域名准备独立到 Phase 1.14。
...
| Phase 1.9A | - | 已完成 | 前端页面布局和 UI/UX 优化 | 设置页信息架构、首页空间创建流程、Banner/背景面板和首页网站收集区编辑入口已收口 | M-L | Medium |
| Phase 1.9B | - | MVP 已完成 | 浏览器收藏/标签导入需求集 | 已完成导入权限边界、大批量导入体验设计和书签 HTML/URL 列表导入 MVP | L | High |
| Phase 1.10 | - | MVP 已完成 | 正式推出前基础收口 | 数据导入/恢复、本地操作审计、本机状态和同步请求多标签协调已接入；账号删除、只读分享链接和密码保护空间保留为候选 | L | High |
| Phase 1.11 | P0-P1 | 已完成 | 数据保全与发布观测体系 | 已建立本地/云端历史版本、数据恢复中心、危险写入保护、同步误覆盖防护、账号托管恢复边界、P0 回归演练指南、基础埋点和错误监控；后台管理 dashboard 延期到 Phase 1.18 | L-XL | High |
| Phase 1.12 | P1 | 后移 | 组件设计和优化 | 复盘 Todo、月历、组件管理模式和模板默认组件，优化配置入口、视觉密度、移动端布局和后续组件候选 | M-L | Medium |
| Phase 1.13 | P2 | 已完成 | 产品化体验收口 | 设置页信息架构 v2、产品身份收口和主题风格 v2 已完成 | M-L | High |
| Phase 1.14 | P0 | 待实现 | 主域名准备 | 域名购买后独立处理 Cloudflare Pages 主站迁移、GitHub Pages 旧站角色、Auth redirect、`basePath`、安全基线和缓存隔离回归 | M-L | High |
