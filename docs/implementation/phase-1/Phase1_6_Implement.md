...
浏览器收藏/标签导入已调整为 Phase 1.9 独立需求集，暂不细化。调整原因：

- 普通网页不能直接读取浏览器收藏夹或当前打开标签页。
- 用户收藏可能有上千条，需要单独设计大批量导入、清洗、去重、分组、抽样预览、性能和回滚策略。
- 是否采用书签 HTML、粘贴 URL 列表、浏览器扩展或多方案组合，留到 Phase 1.9 需求设计阶段决定。

原 Phase 1.9 正式推出前收口整体后移到 Phase 1.10，包括主域名准备、数据导入/恢复、操作审计、设备状态、账号删除、只读分享链接和密码保护空间评估。
...
| Phase 1.9 | P1 | 占位待设计 | 浏览器收藏/标签导入需求集 | 作为独立需求集重新设计；暂不细化具体导入方案 | L | High |
| Phase 1.10 | P1 | 延后 | 主域名准备 | 域名购买决策完成后，在正式推出前处理自购主域名、Auth redirect、`basePath` 和缓存隔离回归 | M | High |
