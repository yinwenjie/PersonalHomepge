...
# Phase 1.3.4 实施计划：同步请求优化与 Revision 回绕控制

## Summary

Phase 1.3.4 优化同步码自动同步机制：新增轻量 revision check，降低完整拉取频率；将本地和云端 revision 统一限制在测试上限 `140`，超过 `140` 后归 `0`，用于验证 rollback 功能。页面刷新后的主动完整拉取保留，数据库轮询频率改为 1 分钟。

## Key Changes

- 本地 `HomeDocumentV2.revision` 和 Supabase `sync_spaces.revision` 都限制为 `0-140`。
- 前端新增统一 revision 规则：`nextRevision(current) = current >= 140 ? 0 : current + 1`。
- 本地待上传判断改为 `revision + updatedAt` 组合判断，避免 revision 回到 `0` 后被旧大小比较挡住。
- 同步绑定状态增加 `lastSyncedDocumentUpdatedAt`，兼容旧绑定缺少该字段的情况。
- 页面刷新或首次加载读取到同步码后，继续执行完整 `pull_sync_space`。
- focus、visibilitychange 和 interval 自动同步先调用 `check_sync_space_revision`。
- 云端 `revision` 或 `updated_at` 变化时才执行完整 pull。
- 自动轮询间隔从 30 秒改为 1 分钟。

## Supabase Changes

- 新增 migration：`002_revision_limit_and_check.sql`。
- `sync_spaces.revision` 约束改为 `revision between 0 and 140`。
- 新增 SQL helper：`next_sync_revision(p_current integer)`。
- `push_sync_space` 和 `force_push_sync_space` 使用 helper 更新 revision。
- 新增只读 RPC：`check_sync_space_revision(p_space_id uuid, p_access_token text)`。
- `check_sync_space_revision` 只返回 `revision`、`updated_at`，不更新 `last_pulled_at`。

## Test Plan

- 本地 revision `139 -> 140 -> 0` 时，保存和自动上传都正常触发。
- 云端 revision `140 -> 0` 后，另一浏览器能通过 check 发现变化并拉取。
- 页面刷新后仍主动完整拉取云端数据。
- 自动轮询每 1 分钟先 check；云端无变化时不完整 pull。
- 本地有修改后 debounce 自动上传。
- 远端和本地都有修改时仍进入 conflict。
- 创建同步码、绑定同步码、手动上传、手动拉取、废弃同步码正常。
- 错误 access token 不能 check、pull 或 push。
- `npm run lint`、`npm run typecheck`、`npm run build` 通过。

## Assumptions

- 当前 revision 上限 `140` 是测试配置，正式上线前应改为 `999`。
- 本阶段不引入 Supabase Realtime。
- 本阶段不改变同步码格式。
- 多标签 leader lock 暂不作为本阶段必做项。

---

# Phase 1.3.5 实施记录：Revision 上限恢复与同步机制收尾

## Summary

Phase 1.3.5 将 Phase 1.3.4 中用于验证回绕逻辑的测试上限 `140` 恢复为正式上限 `999`。本阶段不改变同步码格式、不改变自动同步流程，只统一前端和 Supabase 的 revision 最大值，并补充数据库迁移执行说明。

## Key Changes

- 前端 `SYNC_REVISION_MAX` 从 `140` 调整为 `999`。
- 本地 revision 继续使用统一规则：`nextRevision(current) = current >= 999 ? 0 : current + 1`。
- `0` 继续作为合法 revision，仍通过 `revision + updatedAt` 判断数据是否变化。
- 新增 Supabase migration：`003_revision_limit_999.sql`。
- Supabase `sync_spaces.revision` 约束调整为 `revision between 0 and 999`。
- Supabase `next_sync_revision(p_current integer)` 调整为 `999 -> 0`。
- 保留页面刷新/启动时完整拉取云端数据。
- 保留后台每 1 分钟先 check、变化后再完整 pull 的自动同步策略。

## Supabase Migration Checklist

线上数据库需要按顺序执行：

1. `001_sync_spaces.sql`：创建同步空间表和基础 RPC。
2. `002_revision_limit_and_check.sql`：加入 `0-140` 测试回绕、轻量 revision check RPC。
3. `003_revision_limit_999.sql`：将 revision 上限恢复为正式值 `999`。

如果线上已经执行过 `001` 和 `002`，本阶段只需要执行 `003_revision_limit_999.sql`。

## Test Plan

- 本地 revision `998 -> 999 -> 0` 时，保存和自动上传都正常触发。
- 云端 revision `999 -> 0` 后，另一浏览器能通过 check 发现变化并拉取。
- 页面刷新后仍主动完整拉取云端数据。
- 自动轮询仍为每 1 分钟先 check；云端无变化时不完整 pull。
- 本地有修改后 debounce 自动上传。
- 远端和本地都有修改时仍进入 conflict。
- `npm run lint`、`npm run typecheck`、`npm run build` 通过。
...
- `999` 是当前同步码阶段的正式 revision 上限。
- 本阶段不引入 Supabase Realtime。
- 本阶段不改变同步码格式。
- 本阶段不实现多标签 leader lock。
