...
| 范围 | 当前能力 | 主要问题 | 后续阶段 |
|---|---|---|---|
| WidgetPanel | 可添加 Todo/月历；支持管理模式、重命名、折叠、删除、拖拽排序和上移/下移兜底；Phase 1.12.1 已新增统一 `WidgetShell`；Phase 1.12.4 已新增统一设置入口和配置弹窗 | 后续组件应复用统一设置入口，避免把配置散落到内容区 | 后续组件 |
...
| 月历 | 支持公历月视图、上月/下月、回到今天、今天高亮、周一/周日起始；Phase 1.12.3 已优化月份切换、回今天、窄屏布局和折叠摘要；Phase 1.12.4 已将周起始迁入统一配置弹窗 | 后续如果扩展日程、节假日或外部日历，必须先重新评估数据边界 | 后续候选 |
| 模板默认组件 | 六个模板可预设 Todo/月历，空白首页不预设组件；Phase 1.12.5 已按模板场景调整组件标题、组合和极简模板折叠状态 | 后续新增组件进入模板前必须先评估是否会让新空间过重 | 后续模板迭代 |
...
Phase 1.12.4 已建立统一配置入口和配置弹窗，后续可配置组件应继续遵守：
...
interface WidgetDefinition {
  ...
  // Phase 1.12.4 implemented:
  // settings?: { title: string; description: string };
}
```

Phase 1.12.4 先落地轻量 `settings` 元信息，不实现完整 schema-driven form；等组件数量和配置字段增加后再评估 `WidgetSettingDefinition[]`。
...
- 增加新 widget type 时必须同步更新：
  ...
  - 模板摘要
  - 数据包导出/恢复回归
- 如果某个候选组件需要网络 API、账号权限、Storage 或大体积数据，应参考 Phase 1.12.6 的 `WidgetCandidatesBacklog.md`，先完成价值、数据边界和后端成本评估，不直接实现。
...
| Phase 1.12.3 | 已完成：月历优化保持公历月视图，改善月份切换、回今天、周起始切换、窄屏和折叠摘要 | 不做农历、节假日、日程或外部日历接入 |
| Phase 1.12.4 | 已完成：配置入口统一，Todo/月历使用同一设置弹窗，月历周起始迁入配置面板，配置保存走 `normalizeConfig` 边界 | 不把所有内容操作都塞进设置面板 |
| Phase 1.12.5 | 已完成：模板只调整新建首页的默认组件组合、标题和折叠状态 | 不自动迁移或修改用户已有首页 |
| Phase 1.12.6 | 已完成：候选组件先评估价值、数据边界和后端成本，新增 `WidgetCandidatesBacklog.md` | 不实现复杂组件市场、联网组件或付费组件权益 |
